...
const redirectToSecurityPage = (() => {
  ...
  return () => {
    if (!triggered) {
      triggered = true;
      window.location.href = '/generic-error.html';
    }
  };
})();

// Globally disable context menu interactions
document.addEventListener('contextmenu', event => {
  event.preventDefault();
});

function shouldHijackDevtools(event) {
  ...
  // Common shortcuts: Ctrl+Shift+I/J/C/K/M/S/P, Cmd/Ctrl variants
  if (shiftKey && (ctrlKey || metaKey)) {
    const targeted = ['i', 'j', 'c', 'k', 'm', 's', 'p'];
    if (targeted.includes(comboKey)) {
      return true;
    }
  }

  // Ctrl/Cmd + Option/Alt + key combos
  if ((ctrlKey || metaKey) && altKey && ['i', 'j', 'c', 'k', 'm', 'p'].includes(comboKey)) {
    return true;
  }
